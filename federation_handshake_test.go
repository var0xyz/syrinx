...
func TestFederationHandshake_FullRoundTrip(t *testing.T) {
	...
	seedFederationUser(t, a.ds.db, "a-admin", "a-admin", roles.RoleAdmin)
	seedFederationUser(t, b.ds.db, "b-admin", "b-admin", roles.RoleAdmin)
	...
	inviteID, connectionString := createInvitationEncryptedTo(t, a, "a-admin", b.kp)
	...
	rr := httptest.NewRecorder()
	req := federationWithUID(httptest.NewRequest(http.MethodPost, "/api/federation/attempt", bytes.NewReader(attemptBody)), "b-admin")
...
func TestIncomingFederationAttempt_WrongSecret(t *testing.T) {
	...
	inviteID, _ := createInvitationEncryptedTo(t, a, "a-admin", remoteKP)
...
func TestIncomingFederationAttempt_ReplayNotNew(t *testing.T) {
	...
	hash := crypto.Hash("s")
	if err := a.ds.InsertFederationInvitation(context.Background(), "inv1", "peer", "a-admin", "fp-b", "remote-armor", hash, "cipher-armor", fixed); err != nil {
		t.Fatal(err)
	}
