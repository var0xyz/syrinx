...
(hidden input), validate length, and **store it in the keychain** for later
boots.
...
- Add a small helper (prefer under `syrinx/recovery` or a tiny `syrinx/secret`
  package used by `main` and `cmd/ops`) that resolves the passphrase in order:
  ...
  3. **Prompt** — if stdin is a TTY, prompt (hidden); enforce ≥16 characters;
     store in keychain; return the value for the process lifetime.
...
|--------|----------------|
| Server key passphrase | Env (HA) **or** OS keychain after first prompt; unwraps `privateKeyArmor` |
...
- **Single-host / long-running:** leave env unset; first interactive boot or
  `ops` run prompts and fills the keychain; later restarts are unattended.
...
- [ ] Env unset + empty keychain + TTY → prompt → key stored → `InitServerKey` works
