# Recovery 11 — SPA peer identity report-back
...
- Execute **peer identity** ledger items sequentially:
...
- Build nest per peer from IndexedDB; if a full chain cannot be assembled,
  **skip** that peer (already reflected in 09 counts) without failing the run.
- Update persisted ledger progress after each success or skip.
...
Reuse the key-chain builder from 10 with a different subject user. Caller must
be the claimant (authenticated after 10).
