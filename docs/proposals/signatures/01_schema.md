...
Use the DDL from [00](00_design.md). Prefer `BIGSERIAL` ids. Do not FK
`fingerprint` to `user_keys` / `private_keys` in v1 — historical keys and
server-key rotation already rely on fingerprint lookup tables separately.
