<script lang="ts">
  ...
  async function refresh() {
    try {
      const [att, attemptLogs] = await Promise.all([
        apiService.getFederationAttempt(attemptId),
        apiService.getFederationAttemptLogs(attemptId),
      ]);
      attempt = att;
      logsText = attemptLogs;
    } catch (err) {
      console.error('[mesh/attempt]', err);
      notificationStore.error(err instanceof Error ? err.message : 'Failed to load attempt');
    }
  }
  ...
  async function reject() {
    ...
    rejecting = true;
    try {
      await apiService.rejectFederationAttempt(attemptId, reason);
      showRejectModal = false;
      notificationStore.success('Attempt rejected');
      await refresh();
    } catch (err) {
      notificationStore.error(err instanceof Error ? err.message : 'Failed to reject attempt');
    } finally {
      rejecting = false;
    }
  }
...
<Auth>
  <div class="mesh-container">
    <div class="mesh-content">
      ...
      {:else if attempt}
        ...
        {#if attempt.status === 'rejected'}
          ...
          {#if attempt.rejectedReason}
            <p class="meta reason">&ldquo;{attempt.rejectedReason}&rdquo;</p>
          {/if}
        {/if}
        ...
        {#if !logsText}
          <div class="empty-state">
            ...
            <p>Handshake and decision events for this attempt will show up here.</p>
          </div>
        {:else}
      ...
      {:else}
        <button class="back-link" on:click={goBack}>&larr; Back to mesh</button>
        <p class="muted">Unknown attempt: {attemptId}</p>
      {/if}
    </div>
  ...
  {#if showRejectModal}
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="reject-attempt-title"
      tabindex="-1"
      on:click={(e) => e.target === e.currentTarget && dismissRejectModal()}
      on:keydown={(e) => e.key === 'Escape' && dismissRejectModal()}
    >
      <div class="modal">
        <h2 id="reject-attempt-title">Reject attempt</h2>
        <p class="modal-lead">Explain why this connection is being rejected. This is recorded in the attempt's log.</p>
        <label class="field">
          <span>Reason</span>
          <textarea
