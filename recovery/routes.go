...
func RegisterRoutes(api *mux.Router, deps Deps) {
	...
	api.HandleFunc("/recovery/reeds", deps.ReportReed).Methods(http.MethodPost)
	api.HandleFunc("/recovery/reeds", noop).Methods(http.MethodOptions)
	api.HandleFunc("/recovery/following", deps.ReportFollowing).Methods(http.MethodPost)
	api.HandleFunc("/recovery/following", noop).Methods(http.MethodOptions)
	api.HandleFunc("/recovery/complete", deps.CompleteImport).Methods(http.MethodPost)
	api.HandleFunc("/recovery/complete", noop).Methods(http.MethodOptions)
}
